//! Profile registration and update logic for the Zap402 contract.
...
pub fn register_profile(
    ...
    storage::add_username_to_directory(env, &username);
...
pub fn deregister_profile(env: &Env, caller: Address) -> Result<(), ContractError> {
    ...
    storage::remove_username_from_directory(env, &profile.username);
    storage::decrement_total_creators(env);

    // 4.5: Network ranking removal
    crate::network::remove_from_network(env, &caller);
