//! # Zap402 — creator payout layer on Stellar (Soroban)
//!
//! Layer 1 **Zap**: tips via a configurable Stellar Asset Contract (`send_tip`) — USDC SAC, native XLM SAC, etc.
//! Unlocks and AI Clone monetization are composed in the app; this contract
//! provides profiles, zaps, withdrawals, credit, and network ranking.
//!
//! ## Features
//! - Creator profile registration
//! - Tips with optional messages (amounts in the configured SAC’s smallest units, typically 7 decimals)
//! - Withdrawal with configurable fee (default 2%)
//! - Credit score based on X (Twitter) metrics
//! - On-chain network ranking (top creators)
//!
...
use crate::types::{
    BatchSkip, ContractConfig, ContractStats, CreditBreakdown, CreditTier, NetworkEntry,
    Profile, Tip,
};
...
impl Zap402Contract {
    ...
    /// Initialize the contract with admin, fee collector, fee percentage, and **tip asset** SAC.
    ///
    /// `native_token` is the Stellar Asset Contract id for `send_tip` / `withdraw_tips` (e.g. USDC SAC).
    /// Amounts use that asset’s smallest units (7 decimals for Circle USDC and native XLM).
    ...
    /// - Removes creator from network ranking (if present)
    ...
    /// Return an ordered paginated list of registered usernames.
    ///
    /// - `offset`: number of usernames to skip from the start.
    /// - `limit`: maximum usernames to return (`0` returns empty).
    pub fn get_all_usernames(env: Env, offset: u32, limit: u32) -> Vec<String> {
        storage::get_usernames_page(&env, offset, limit)
    }

    /// Return an ordered paginated list of full profiles.
    ///
    /// This is derived from `get_all_usernames` and only includes active profiles.
    pub fn get_profiles_page(env: Env, offset: u32, limit: u32) -> Vec<Profile> {
        let usernames = storage::get_usernames_page(&env, offset, limit);
        let mut out: Vec<Profile> = Vec::new(&env);
        let mut i: u32 = 0;
        while i < usernames.len() {
            let username = usernames.get(i).unwrap();
            if let Some(address) = storage::get_username_address(&env, &username) {
                if storage::is_profile_active(&env, &address) {
                    out.push_back(storage::get_profile(&env, &address));
                }
            }
            i += 1;
        }
    ...
    /// Send a tip to a registered creator (pulls the configured SAC into escrow).
    pub fn send_tip(
        env: Env,
        tipper: Address,
        creator: Address,
    ...
    pub fn get_tip(env: Env, tip_id: u32) -> Result<Tip, ContractError> {
        zaps::get_tip(&env, tip_id).ok_or(ContractError::NotFound)
    }
    ...
    pub fn get_recent_tips(env: Env, creator: Address, limit: u32, offset: u32) -> Vec<Tip> {
        zaps::get_recent_tips(&env, &creator, limit, offset)
    }
    ...
    pub fn get_tips_by_tipper(env: Env, tipper: Address, limit: u32) -> Vec<Tip> {
        zaps::get_tips_by_tipper(&env, &tipper, limit)
    }
    ...
    pub fn get_network(env: Env, limit: u32) -> Result<Vec<NetworkEntry>, ContractError> {
        Ok(network::get_network(&env, limit))
    }

    /// Return the 1-based rank of `address` in the network ranking, or `None`
    /// when the address has not yet appeared in the top 50.
    pub fn get_network_rank(env: Env, address: Address) -> Option<u32> {
        network::get_network_rank(&env, &address)
    }

    /// Return the current number of entries in the network ranking (0–50).
    pub fn get_network_size(env: Env) -> u32 {
        network::get_network_size(&env)
    }
    ...
    /// Point tips and withdrawals at a different Stellar Asset Contract (e.g. switch to USDC SAC).
    ///
    /// Admin only. See [`admin::set_tip_asset`] for migration safety notes.
    pub fn set_tip_asset(
        env: Env,
        caller: Address,
        new_token: Address,
    ) -> Result<(), ContractError> {
        admin::set_tip_asset(&env, &caller, &new_token)
    }
