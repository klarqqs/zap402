...
fn setup() -> (Env, Zap402ContractClient<'static>) {
    let env = Env::default();
    env.mock_all_auths();
...
fn register_test_profile(env: &Env, client: &Zap402ContractClient, username: &str) -> Address {
...
#[test]
fn get_all_usernames_returns_paginated_directory() {
    let (env, client) = setup();
    register_test_profile(&env, &client, "alice");
    register_test_profile(&env, &client, "bob");
    register_test_profile(&env, &client, "charlie");

    let page = client.get_all_usernames(&1_u32, &2_u32);
    assert_eq!(page.len(), 2);
    assert_eq!(page.get(0).unwrap(), String::from_str(&env, "bob"));
    assert_eq!(page.get(1).unwrap(), String::from_str(&env, "charlie"));
}

#[test]
fn get_profiles_page_returns_paginated_profiles() {
    let (env, client) = setup();
    register_test_profile(&env, &client, "alice");
    register_test_profile(&env, &client, "bob");
    register_test_profile(&env, &client, "charlie");

    let page = client.get_profiles_page(&0_u32, &2_u32);
    assert_eq!(page.len(), 2);
    assert_eq!(page.get(0).unwrap().username, String::from_str(&env, "alice"));
    assert_eq!(page.get(1).unwrap().username, String::from_str(&env, "bob"));
}

#[test]
fn directory_excludes_deregistered_profiles() {
    let (env, client) = setup();
    let alice = register_test_profile(&env, &client, "alice");
    let _bob = register_test_profile(&env, &client, "bob");

    client.deregister_profile(&alice);

    let usernames = client.get_all_usernames(&0_u32, &10_u32);
    assert_eq!(usernames.len(), 1);
    assert_eq!(usernames.get(0).unwrap(), String::from_str(&env, "bob"));
}
