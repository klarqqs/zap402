//! Tests for network ranking functionality (issue #28).
//!
//! ## Coverage
//! - [`test_network_initial_empty`]   — no tips → empty board
//! - [`test_network_single_creator`]  — one tipped creator appears
//! - [`test_network_ordering`]        — multiple creators sorted descending
//! - [`test_network_max_size`]        — 51 creators → only top 50 retained
//! - [`test_network_rank_update`]     — creator moves up after more tips
//! - [`test_network_rank_lookup`]     — `get_network_rank` returns correct 1-based position
//!
//! Tests that require XLM transfers use the full SAC setup (same pattern as
//! `test_zaps.rs`).  The `test_network_max_size` test bypasses `send_tip`
//! and calls `network::update_network` directly inside the contract
...
use crate::network::MAX_NETWORK_SIZE;
use crate::storage::DataKey;
use crate::types::Profile;
use crate::Zap402Contract;
use crate::Zap402ContractClient;
...
/// tests stay fast and focused on network ranking behaviour).
...
/// Before any tips are sent the network ranking must be empty.
#[test]
fn test_network_initial_empty() {
    let (env, client, contract_id, _, _) = setup();
    let board = client.get_network(&50);
    assert_eq!(
        board.len(),
        0,
        "network ranking should be empty before any tips"
    );

    let stranger = Address::generate(&env);
    env.as_contract(&contract_id, || {
        assert!(!crate::network::is_on_network(&env, &stranger));
    });
}

/// A single creator who has received a tip must appear on the network ranking with
/// the correct total.
#[test]
fn test_network_single_creator() {
    ...
    env.as_contract(&contract_id, || {
        assert!(crate::network::is_on_network(&env, &creator));
    });
}

/// With three creators the network ranking must be ordered descending by
/// `total_tips_received`.
#[test]
...
///
/// This test calls `network::update_network` directly inside the
/// contract context to avoid the cost of 51 token-transfer transactions.
#[test]
fn test_network_max_size() {
    let env = Env::default();
    let contract_id = env.register_contract(None, Zap402Contract);
    let client = Zap402ContractClient::new(&env, &contract_id);
    let now = env.ledger().timestamp();
    ...
    // Populate the network ranking directly via the internal function.
    ...
    assert_eq!(
        board.len(),
        MAX_NETWORK_SIZE,
        "network ranking must retain at most {MAX_NETWORK_SIZE} entries"
    );
    ...
    env.as_contract(&contract_id, || {
        assert!(!crate::network::is_on_network(&env, &lowest));
        assert!(crate::network::is_on_network(
            &env,
...
fn test_network_rank_update() {
    ...
    let board_before = client.get_network(&50);
    assert_eq!(
    ...
    let board_after = client.get_network(&50);
    assert_eq!(
...
/// `get_network_rank` must return the correct 1-based position for every
/// ranked creator and `None` for an address not on the network ranking.
#[test]
fn test_network_rank_lookup() {
    let (env, client, contract_id, tipper, _) = setup();
    ...
    assert_eq!(client.get_network_rank(&carol), Some(1));
    assert_eq!(client.get_network_rank(&alice), Some(2));
    assert_eq!(client.get_network_rank(&bob), Some(3));

    // An address that has never received a tip must not be ranked.
    let stranger = Address::generate(&env);
    assert_eq!(
        client.get_network_rank(&stranger),
        None,
