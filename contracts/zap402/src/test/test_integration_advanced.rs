...
fn test_multi_user_tipping_round_robin() {
    ...
    let leaderboard = client.get_network(&10);
...
fn test_rapid_tips_same_creator() {
    let (env, client, _admin, _fee_collector, token_address) = setup_env();
    // Many tips exhaust the default host budget (network ranking work per tip).
...
#[test]
fn test_network_ranking_overtake() {
    let (env, client, _admin, _fee_collector, token_address) = setup_env();
    ...
    // Verify Alice is #1
    let leaderboard = client.get_network(&2);
    ...
    // Verify Bob is #1
    let leaderboard_after = client.get_network(&2);
