...
#[test]
fn test_set_tip_asset_updates_stored_sac() {
    let ctx = setup();
    let new_sac = ctx
        .env
    ...
    ctx.client.set_tip_asset(&ctx.admin, &new_sac);
    ...
    assert_eq!(stored, new_sac);
}

#[test]
fn test_set_tip_asset_non_admin_returns_not_authorized() {
    let ctx = setup();
    ...
    let new_sac = ctx
        ...
        .register_stellar_asset_contract_v2(Address::generate(&ctx.env))
        .address();
    let result = ctx.client.try_set_tip_asset(&attacker, &new_sac);
...
fn setup_initialized() -> (Env, Zap402ContractClient<'static>, Address) {
    let ctx = setup();
