...
> Conventions, patterns, and structure for the Zap402 React frontend.
...
| `borderWidth.3` | `3px` |
| `fontFamily.display` | `Space Mono`, `ui-monospace`, `monospace` |
| `fontFamily.sans` / `body` | `Space Mono`, `ui-monospace`, `monospace` (via `var(--font-body)` in `index.scss`) |
...
import ZapPage from './features/zap402/ZapPage';
