/**
 * Verify Stellar ask payments:
 * - Preferred: Zap402 `send_tip` with message `ask:<messageHash>` (USDC into contract escrow).
 * - Legacy: direct Soroban USDC `transfer` fan → creator.
 */
...
/**
 * Zap402 `send_tip(tipper, creator, amount, message)` — message must be exactly `ask:<hash>`.
 */
function sorobanZap402SendTipMatches(op, opts) {
  ...
  try {
    ...
    if (tipper !== opts.fan || creator !== opts.creator) return false;
    const amtVal = scValToNative(args[2]);
    const paid =
      typeof amtVal === "bigint" ? amtVal : BigInt(Math.floor(Number(amtVal)));
    ...
    const expected = `ask:${opts.messageHash}`;
...
function sorobanTransferMatches(op, opts) {
  ...
  try {
    ...
    if (from !== opts.fan || to !== opts.creator) return false;
    const amtVal = scValToNative(args[2]);
    const paid =
      typeof amtVal === "bigint" ? amtVal : BigInt(Math.floor(Number(amtVal)));
 ...
 * @param {string} p.messageHash - 64 lowercase hex chars
 * @param {number} p.expectedAmount - human USDC
 * @param {'USDC'} p.currency
 */
export async function verifyAskTransaction(p) {
  ...
  if (tx.source_account !== p.fan) {
    return { ok: false, reason: "source_not_fan" };
  }

  const tokenId = (p.tokenContractId || "").trim();
  const zapId = (p.zap402ContractId || "").trim();
  if (!tokenId && !zapId) {
    return { ok: false, reason: "ask_verify_unconfigured" };
  }
